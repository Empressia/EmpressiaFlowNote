...
また、フローチャートから、[ソースコードの各行へのリンクも作成できます](#リンクの作成について)。  
リンクを含むサンプルは、[GitHubのサンプルリポジトリ](https://github.com/Empressia/EmpressiaFlowNoteSample01/)においてあります。  
...
リンクのテンプレート（と、リンク用の基準となるパス）を指定することで、  
フローチャートから、ソースコードの各行へのリンクを自動で作成します。  

任意の形式のリンクを作成できますが、  
ここでは、GitHubに置いてあるソースコードへのリンクの作成を例に説明します。  

コマンドラインで作成するには、  
オプションとして、『-LinkTemplate』と『-LinkBasePath』を使用します。  
```
-LinkTemplate "http://github.com/[ユーザー名]/[リポジトリ名]/blob/[ブランチ名・タグ名・コミットIDなど]/{Path}#L{Line}"
-LinkBasePath "[作業中のGitのリポジトリパス]"
```

これにより、各ソースコードのパスが、LinkBasePathを基準とした相対パスとして『{Path}』に埋め込まれ、リンクが作成されます。  
CIなどで実行することで、任意のコミットIDやタグを対象にしてリンクを作れるかと思います。  

出力されるファイルからの相対パスを、  
```
-LinkTemplate "{Path}#L{Line}"
```
など、『-LinkBasePath』を省略することで作成できますが、  
現状（2026-05-31時点）の、GitHubやVisual Studio Codeの、MarkdownのMermaid表示では、  
リンクの実際の配置場所や制限により、期待した動作はしないようです。  

また、Javaのプログラム内から、リンクの作成を設定するには、  
以下のメソッドを呼び出すことで設定できます。  

* MermaidMarkdownWriter#linkTemplate
* MermaidMarkdownWriter#linkBasePath
